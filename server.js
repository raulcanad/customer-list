...
app.get('/api/mmtexas', async (req, res) => {
    ...
    try {
      connection = await getConnection();
      const result = await connection.execute('SELECT DISTINCT m.idmvversion FROM MMVERSION.MVCURRENTCOMPONENTVERSION m WHERE m.componentname = \'MMTEXAS\' AND m.tscreated = (SELECT MAX(mv.tscreated) FROM MMVERSION.MVCURRENTCOMPONENTVERSION mv WHERE mv.componentname = m.componentname)');
