...
const ScreenComponent: React.FC = () => {
  const [uatVersion, setUatVersion] = useState<ScreenData | null>(null);
  const [prodVersion, setProdVersion] = useState<ScreenData | null>(null);
  ...
  const filterSubmodules = (data: ScreenData): ScreenData => {
    const cleanData: ScreenData = {};
  ...
  const hasSubmodules = (data: ScreenData): boolean => {
    for (const key in data) {
  ...
  return (
    <div className="screen-component">
     <h1 className="title">SCREEN'S VERSIONS</h1>
      {showSubmodulePopup && (
        <div className="popup">
